...
# Identify target column. It's usually 'FloodProbability', 'Target', or 'class'
target_col = [c for c in df.columns if 'landslide' in c.lower() or 'target' in c.lower() or 'class' in c.lower()][-1]
print(f"Using target column: {target_col}")

X = df.drop(target_col, axis=1)
y = df[target_col]

# Map categorical labels to binary for classification
if y.dtype == 'object' or y.dtype.name == 'category' or y.dtype == 'string':
    # Specifically for landslide dataset ('High', 'Very High' -> 1, else 0)
    y = y.isin(['High', 'Very High']).astype(int)
elif y.dtype == 'float64' or y.dtype == 'float32':
...
model_path = 'd:/aapda-setu/backend/ml/landslide_model.pkl'
if best_auc > 0.65:
    print(f"Result: {best_name} achieved AUC={best_auc:.3f} â€” [PROMOTED to YELLOW]")
    os.makedirs(os.path.dirname(model_path), exist_ok=True)
    joblib.dump(best_model, model_path)
else:
    print(f"Result: {best_name} achieved AUC={best_auc:.3f} â€” [NOT PROMOTED, staying deterministic-only]")