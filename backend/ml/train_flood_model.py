...
dataset_path = kagglehub.dataset_download("naiyakhalid/flood-prediction-dataset")
...
target_col = [c for c in df.columns if 'flood' in c.lower() or 'target' in c.lower() or 'class' in c.lower()][-1]
print(f"Using target column: {target_col}")

X = df.drop(target_col, axis=1)
y = df[target_col]

# If target is continuous (probability), threshold it to binary for classification
...
model_path = 'd:/aapda-setu/backend/ml/flood_model.pkl'
if best_auc > 0.65:
    print(f"Result: {best_name} achieved AUC={best_auc:.3f} — [PROMOTED to YELLOW]")
